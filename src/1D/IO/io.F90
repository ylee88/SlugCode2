...
contains
  ...
  subroutine io_writeHDF5_p(init_file_name, t, nstep, ioCounter, eTime, commit_hash)
    ...
    rank_V     = 2
    dims_V     = (/ NSYS_VAR, gr_nx     /)
    glb_dims_V = (/ NSYS_VAR, gr_glb_nx /)
...
910 format(i5)
  ...
  subroutine io_writeHDF5(init_file_name, t, nstep, ioCounter, eTime, commit_hash)
    ...
    real, dimension(NUMB_VAR, gr_glb_nx) :: V
    ...
    if (bl_ID == 0) then
      ...
      V(:,1:gr_nx) = gr_V(:,gr_ibeg(XDIM):gr_iend(XDIM))
      ...
      if (bl_nProcs > 1) then
        do i = 1, bl_iProcs
          ...
          if (dest /= 0) then
            ...
            call MPI_Recv( V(:,ibeg:iend), NUMB_VAR*gr_nx, MPI_DOUBLE_PRECISION, &
      ...
      call MPI_SEND( gr_V(:,gr_ibeg(XDIM):gr_iend(XDIM)), &
                     NUMB_VAR*gr_nx, MPI_DOUBLE_PRECISION, 0, tag, MPI_COMM_WORLD, ierr)
...
910 format(i5)
  ...
  subroutine io_writeASCII(t, nstep, ioCounter)
    ...
    if (bl_ID == 0) then
      ...
      do i = 1, gr_glb_nx
        write(20,920)xCoord(i),(V(nVar,i),nVar=1,EINT_VAR)
      end do
      close(20)
