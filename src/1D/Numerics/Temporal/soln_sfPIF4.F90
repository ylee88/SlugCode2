...
#include "definition.h"
  ...
  real, dimension(NSYS_VAR) :: Ft, Ftt, Fttt
  real, dimension(NSYS_VAR) :: div, divx, divt

  real, dimension(NSYS_VAR) :: Fxtt

  real, dimension(NSYS_VAR) :: divxx
  real, dimension(NSYS_VAR) :: Fxxt
  real, dimension(NSYS_VAR) :: divtx, divtt
  ...
  do i = ibeg-(num_radius+1), iend+(num_radius+1)
    ...
    divxx = diff2(div_gr(:, i-2:i+2), 5, gr_dx)
    ...
    Flux(:, i, XDIM) = Flux(:, i, XDIM) + dt**3/24.*Fttt(:)
