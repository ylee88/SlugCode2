...
#include "definition.h"
  ...
  allocate(gr_U(NSYS_VAR, gr_imax(XDIM), gr_imax(YDIM))); gr_U = 0.
  allocate(gr_V(NUMB_VAR, gr_imax(XDIM), gr_imax(YDIM))); gr_V = 0.

  ! allocate Riemann states
  allocate(gr_vL(NUMB_VAR, gr_imax(XDIM), gr_imax(YDIM), NDIM)); gr_vL = 0.
  allocate(gr_vR(NUMB_VAR, gr_imax(XDIM), gr_imax(YDIM), NDIM)); gr_vR = 0.

  ! allocate interfacial fluxes
  allocate(gr_fL(NSYS_VAR, gr_imax(XDIM), gr_imax(YDIM), NDIM)); gr_fL = 0.
  allocate(gr_fR(NSYS_VAR, gr_imax(XDIM), gr_imax(YDIM), NDIM)); gr_fR = 0.

  ! allocate grid fluxes
  allocate(gr_flux(NSYS_VAR, gr_imax(XDIM), gr_imax(YDIM), NDIM)); gr_flux = 0.
