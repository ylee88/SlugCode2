...
contains
  ...
  subroutine io_writeHDF5_p(init_file_name, t, nstep, ioCounter, eTime)
    ...
    real, allocatable, dimension(:)         :: yCoord, xCoord, zCoord

    integer(HSIZE_T), dimension(1) :: dimT, dims_XYZ
    integer(HSIZE_T), dimension(3) :: xyoffset
    ...
    allocate(img_V(NSYS_VAR, gr_nx, gr_ny, gr_nz))
    ...
    rank_V     = 4
    dims_V     = (/ NSYS_VAR, gr_nx,     gr_ny,     gr_nz     /)
    glb_dims_V = (/ NSYS_VAR, gr_glb_nx, gr_glb_ny, gr_glb_nz /)
    ...
    ! if (bl_ID == 1) then
      ...
      ! close dataspace && dataset for z-coordinates
      call H5Dclose_f(dset_id,error)
      call H5Sclose_f(dspace_id,error)
...
910 format(i5)
  ...
  subroutine io_writeHDF5(init_file_name, t, nstep, ioCounter, eTime)
    ...
    if (bl_ID == 0) then
      ...
      V(:,1:gr_nx,1:gr_ny,1:gr_nz) = gr_V(:,gr_ibeg(XDIM):gr_iend(XDIM), &
                                            gr_ibeg(YDIM):gr_iend(YDIM), &
                                            gr_ibeg(ZDIM):gr_iend(ZDIM)  )
          ...
          do i = 1, bl_iProcs
            do j = 1, bl_jProcs
              ...
              if (dest /= 0) then
                ...
                call MPI_Recv( V(:,ibeg:iend, jbeg:jend, kbeg:kend), NUMB_VAR*gr_nx*gr_ny*gr_nz, MPI_DOUBLE_PRECISION, &
      ...
      call MPI_SEND( gr_V(:,gr_ibeg(XDIM):gr_iend(XDIM), gr_ibeg(YDIM):gr_iend(YDIM), gr_ibeg(ZDIM):gr_iend(ZDIM)), &
                     NUMB_VAR*gr_nx*gr_ny*gr_nz, MPI_DOUBLE_PRECISION, 0, tag, MPI_COMM_WORLD, ierr)
...
910 format(i5)
  ...
  subroutine io_writeASCII(t, nstep, ioCounter)
    ...
    if (bl_ID == 0) then
      ...
      V(:,1:gr_nx,1:gr_ny,1:gr_nz) = gr_V(:,gr_ibeg(XDIM):gr_iend(XDIM), gr_ibeg(YDIM):gr_iend(YDIM), gr_ibeg(ZDIM):gr_iend(ZDIM))
      ...
      do k = 1, gr_glb_nz
        do j = 1, gr_glb_ny
          do i = 1, gr_glb_nx
            write(20,920)xCoord(i),yCoord(j),zCoord(k),(V(nVar,i,j,k),nVar=1,EINT_VAR)
          end do
          write(20,100)
        end do
      end do
